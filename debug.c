...
print_readable( const unsigned char *data, unsigned int len,
  ...
  // iskim
  // printf("\n__%s__(__%d__), ", __FILE__, __LINE__);
  ...
  */
  ...
  while ((option = coap_option_next(&opt_iter))) {
    ...
    if (print_readable(COAP_OPT_VALUE(option), 
      ...
      // iskim fprintf(COAP_DEBUG_FD, " %d:'%s'", opt_iter.type, buf);
      fprintf(COAP_DEBUG_FD, "__%s__(__%d__), %d:'%s'", __FILE__, __LINE__, opt_iter.type, buf);
  }
  ...
  if (pdu->data) {
    ...
    print_readable(pdu->data, 
		   ...
		   buf, sizeof(buf), 0 );
    // iskim fprintf(COAP_DEBUG_FD, " d:%s", buf);
    printf("\n__%s__(__%d__), d:%s", __FILE__, __LINE__, buf);
  }
  fprintf(COAP_DEBUG_FD, "\n");
